<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="articleedit.aspx.cs" Inherits="QiZhanBang.Web.backend.articleedit" ValidateRequest="false" EnableEventValidation="false" %>
...
-->
	...
	<script charset="utf-8" src="../js/jquery-1.8.3.min.js" type="text/javascript"></script>
...
<tr>
  ...
  <table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" id="tagContent0">
	...
	<tr>
      ...
      <td class="tdbg"><asp:DropDownList runat="server" ID="ddlArticleCategory" DataTextField="categoryname" DataValueField="categoryid"></asp:DropDownList> &nbsp;<asp:DropDownList 
              runat="server" ID="ddlArticleCategorySub"></asp:DropDownList> 　<asp:CheckBox runat="server" ID="chkiscommend"  /><label for="t6">推荐</label> <asp:CheckBox runat="server" ID="chkistop" /><label for="t7">置顶</label> <asp:CheckBox runat="server" ID="chkischecked" Checked="true" /><label for="t8">审核</label></td>
    </tr>
    <tr>
   ...
   <tr class="tdbg" id="flag3">
      ...
      <td><textarea name="editorcontent" id="editorcontent" style="width:800px;height:500px;visibility:hidden;" runat="server"></textarea></td>
    </tr>
  </table>  
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" >
    ...
    <tr class="tdbg">
      ...
      <td>
          <asp:TextBox ID="txtcreatetime" runat="server" Width="175px"></asp:TextBox>
        </td>
	  </tr>
    <tr class="tdbg">
	  <td width="100">&nbsp;</td>
      <td style="padding-left:300px;"><asp:Button runat="server" CssClass="bnt" Text="保 存" ID="btnSave" onclick="btnSave_Click" /> </td>
	  </tr>
    <tr class="tdbg">
	  <td width="100">&nbsp;</td>
  ...
  <input type="hidden" id="hdnSmallCatId" name="hdnSmallCatId" runat="server" value="0" />
  <script type="text/javascript" language="javascript">
    ...
    $(function () {
        $('#ddlArticleCategorySub').append($('<option></option>').val('').html('-请选择-'));
        var CateBigId = $("#ddlArticleCategory").val();
        GetArticleCategorySub(CateBigId);

        $("#ddlArticleCategory").change(function () {
            var CateBigId = $("#ddlArticleCategory").val();
            GetArticleCategorySub(CateBigId);
        });
    });

    function GetArticleCategorySub(CateBigId) {
        $.ajax({
            url: "../Handler/Category.ashx?Method=GetArticleCategorySubList",
            data: { CateBigId: CateBigId },
            type: "get",
            dataType: "json",
            success: function (data) {
                $('#ddlArticleCategorySub option').each(function (i, option) { $(option).remove(); });
                $('#ddlArticleCategorySub').append($('<option></option>').val('').html('-请选择-'));
                if (data != null && data.rows != null) {
                    $.each(data.rows, function (i, item) {
                        $('#ddlArticleCategorySub').append($('<option></option>').val(item.categoryid).html(item.categoryname));
                    });
                    $("#ddlArticleCategorySub").val($("#hdnSmallCatId").val());
                }
            },
            error: function () {
                alert("服务器错误！");
